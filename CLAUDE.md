...
## Identity
- **Vessel**: Capitaine
- **Role**: Capitaine — fork to alive
- **URL**: https://capitaine.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/capitaine
- **Branch**: master
...
### Deploy
```bash
cd /tmp/capitaine && wrangler deploy
```

### Health Check
```bash
curl -s https://capitaine.casey-digennaro.workers.dev/health
curl -s https://capitaine.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=capitaine
...
- **vessel.json capabilities**: agent, github, fleet
...
1. Keep the vessel small. ~514 lines is the current size.
